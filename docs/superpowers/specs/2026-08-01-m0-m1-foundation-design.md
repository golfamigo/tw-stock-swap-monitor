...
Ownership is scope-based: SYSTEM global reference data, USER private profiles and groups, and PORTFOLIO plans, positions, runs, and decisions. Repositories take AccessContext and fail closed. No table receives a mechanical user_id or portfolio_id when its data is global. Position history is retained through a partial open-position uniqueness rule.
...
Rules are a restricted, schema-validated JSON-Logic subset with bounded tree depth and evidence capture. Sparse LayerPatchSchema inputs merge into a distinct required ResolvedConfigurationSchema, whose versioned canonical serialization produces a reproducible hash. Idempotency uses a pre-fetch scan lock and a post-fetch strategy-run identity, so a market snapshot cannot be required before it exists. Run-once and scheduling share one coordinator and lock contract. Protected positions are rejected by domain validation, sizing, and transition guards.
